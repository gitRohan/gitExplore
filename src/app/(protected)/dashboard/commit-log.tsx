            ...
            <ul className="space-y-6">
                {commits?.map((commit,commitIdx)=>{
                    return<li key={commit.id} className="relative flex gap-x-4">
                        <div className={cn(
                            commitIdx === (commits?.length) - 1 ? 'h-6' : '-bottom-10',
                            'absolute left-1 top-6 flex w-6 justify-center'
                        )}>
                            <div className="w-px bg-gray-200"/>
                        </div>
                        <>
                            <img src={commit.commitAuthorAvatar} alt='commit avatar' className="relative mt-4 size-8 rounded-full bg-gray-50"/>
